...
class SiteService {
  ...
  constructor() {
    ...
    this.updateTargets[API.AVAILABLE_ENDPOINTS.FLOW.key] = this.updatePowerFlow
      .bind(this);
    this.updateTargets[API.AVAILABLE_ENDPOINTS.STORAGE.key] = this.updateStorageInformation
      .bind(this);
  ...
  updateSite(id) {
    ...
    for (index = 0; index < updateTargets.length; index++) {
      if (this.updateTargets[updateTargets[index]]) {
        lastUpdate = this.sites[id].site.getTimeOfLastUpdateForTarget(updateTargets[
          index]);
        ...
        if (!lastUpdate || (now - lastUpdate) >= defaultUpdateInterval) {
          ...
          console.log("Information for [" + updateTargets[index] +"] requested but cached version will be served.");
        }
      }
    }
  ...
  updateEnergyInformation(site) {
    var that = this,
      updatePromise = new Promise(function(resolve, reject) {
        ...
        request(route, function(error, response, body) {
          var result = JSON.parse(body).energyDetails;
          resolve({
  ...
  updatePowerFlow(site) {
    var updatePromise = new Promise(function(resolve, reject) {
      ...
      request(route, function(error, response, body) {
        var result = JSON.parse(body).siteCurrentPowerFlow;
        resolve({
  ...
  updateStorageInformation(site) {
    var that = this,
      updatePromise = new Promise(function(resolve, reject) {
        ...
        request(route, function(error, response, body) {
          var result = JSON.parse(body).storageData;
          resolve({
